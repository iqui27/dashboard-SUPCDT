...
async function createParlamentaresTable() {
  ...
  try {
    ...
    const parlamentaresToInsert: Parlamentar[] = parlamentaresNormalizados.map(nome => ({
      nome,
    ...
    const parlamentaresMap = new Map<string, ObjectId>();
    ...
    for (const project of projects) {
      const emendasParlamentaresRef: { parlamentarId: ObjectId; valor?: number }[] = [];
...
function separarENormalizar(texto: string): string[] {
  if (!texto) return [];
  
  // Casos especiais de múltiplos
  const multiplasMap: Record<string, string[]> = {
