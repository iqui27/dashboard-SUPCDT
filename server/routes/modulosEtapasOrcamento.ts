 ...
 * ETAP-01
 */
router.post('/:id/etapas', requireAuth, async (req: Request, res: Response) => {
  try {
    const { id } = req.params;
    ...
    // Criar nova etapa
    const novaEtapa: DBEtapa = {
      id: randomUUID(),
      nome,
      percentual: percentual ?? 0,
      entregaveis: (entregaveis ?? []).map((e: { nome: string }) => ({
        id: randomUUID(),
        nome: e.nome,
        concluido: false
      }))
 ...
 * ETAP-02
 */
router.put('/:id/etapas/:etapaId', requireAuth, async (req: Request, res: Response) => {
  try {
    ...
    if (nome !== undefined) setObj['etapas.$[elem].nome'] = nome;
    if (percentual !== undefined) setObj['etapas.$[elem].percentual'] = percentual;
 ...
 * ETAP-03
 */
router.delete('/:id/etapas/:etapaId', requireAuth, async (req: Request, res: Response) => {
  try {
    const { id, etapaId } = req.params;
 ...
 * ETAP-04
 */
router.patch('/:id/etapas/:etapaId/entregaveis/:entId', requireAuth, async (req: Request, res: Response) => {
  try {
 ...
 * ETAP-05 (adicionar)
 */
router.post('/:id/etapas/:etapaId/entregaveis', requireAuth, async (req: Request, res: Response) => {
  try {
    const { id, etapaId } = req.params;
    const { nome } = req.body;
    ...
    const novoEntregavel: DBEntregavel = {
      id: randomUUID(),
      nome,
      concluido: false
    };
 ...
 * ETAP-05 (remover)
 */
router.delete('/:id/etapas/:etapaId/entregaveis/:entId', requireAuth, async (req: Request, res: Response) => {
  try {
    const { id, etapaId, entId } = req.params;
 ...
 * ORÇA-01
 */
router.post('/:id/rubricas', requireAuth, async (req: Request, res: Response) => {
  try {
    const { id } = req.params;
    ...
    // Criar nova rubrica
    const novaRubrica: DBRubricaOrcamentaria = {
      id: randomUUID(),
      nome,
      previsto: previsto ?? 0,
 ...
 * ORÇA-02
 */
router.put('/:id/rubricas/:rubricaId', requireAuth, async (req: Request, res: Response) => {
  try {
    ...
    // Construir $set dinamicamente
    const setObj: Record<string, unknown> = {};
    if (nome !== undefined) setObj['rubricas.$[elem].nome'] = nome;
    if (previsto !== undefined) setObj['rubricas.$[elem].previsto'] = previsto;
    if (executado !== undefined) setObj['rubricas.$[elem].executado'] = executado;
 ...
 * ORÇA-03
 */
router.post('/:id/rubricas/:rubricaId/aditivos', requireAuth, async (req: Request, res: Response) => {
  try {
    const { id, rubricaId } = req.params;
    ...
    // Criar novo aditivo
    const novoAditivo: DBAditivoRubrica = {
      id: randomUUID(),
      descricao,
      valor,
 ...
 * ORÇA-04
 */
router.delete('/:id/rubricas/:rubricaId', requireAuth, async (req: Request, res: Response) => {
  try {
    const { id, rubricaId } = req.params;
 ...
 * MODU-01
 */
router.patch('/:id/modulos', requireAuth, async (req: Request, res: Response) => {
  try {
    const { id } = req.params;
    const body = req.body;
