...
import {
  SeiDocumentBatchSchema,
  SeiExtractionResultWithMovements,
  SeiDocument,
  SeiUpdateProposal
} from '../types/sei.js';
import {
  extractFromSeiDocuments
} from '../services/seiExtractor.js';
import {
  getSeiProcess,
  processSeiExtraction,
  updateExtractedDocLinks
} from '../services/seiPersistence.js';
import {
  extractSeiProjectSuggestion
} from '../services/seiProjectExtractor.js';
import {
  persistSuggestion
} from '../services/autoPopulate.js';
import {
  ...
  listUpdateProposals,
  getUpdateProposal
} from '../services/seiUpdateProposal.js';
...
seiRouter.post('/import', (req, res, next) => {
  ...
  try {
    ...
    const linkedProjectId: string | undefined = req.body.linkedProjectId || undefined;
    ...
    const alreadyExtracted = new Set<string>(existingProcess?.extractedDocLinks ?? []);
    ...
    const documentsWithProcesso: SeiDocument[] = validated.documents.map(doc => ({
      tipo: doc.tipo,
      link: doc.link,
      nome: doc.nome,
    ...
    const persistenceResult = await processSeiExtraction(
    ...
    const response: SeiExtractionResultWithMovements & {
      autoPopulate?: {
      ...
      updateProposal?: {
        status: 'created' | 'skipped' | 'failed' | 'no_changes';
        proposalId?: string;
    ...
    const hasRelevantMovements =
      persistenceResult.movementResult.isFirstExtraction ||
      persistenceResult.movementResult.movements.some(m =>
        m.type === 'document_added' || m.type === 'document_modified'
      );
    ...
    if (shouldExtract) {
      ...
      try {
        const extractionResult = await extractSeiProjectSuggestion(
          validated.processoSEI,
          newDocsForExtraction,
        ...
        if (extractionResult.success && extractionResult.suggestion) {
          ...
          await updateExtractedDocLinks(
            validated.processoSEI,
            newDocsForExtraction.map(d => d.link).filter(Boolean)
          );

          if (effectiveLinkedProjectId) {
            // ── Update flow: create a diff proposal ──────────────────────
            const proposalResult = await createUpdateProposal(
              validated.processoSEI,
              effectiveLinkedProjectId,
              extractionResult.suggestion,
              newDocsForExtraction.map(d => d.link),
              {
...
seiRouter.get('/update-proposals', (req, res, next) => {
  ...
  try {
    ...
    const result = await listUpdateProposals({
      status: status as SeiUpdateProposal['status'] | undefined,
      processoSEI: processoSEI as string | undefined,
      linkedProjectId: linkedProjectId as string | undefined,
...
seiRouter.get('/update-proposals/:id', (req, res, next) => {
  ...
  try {
    const proposal = await getUpdateProposal(req.params.id as string);
    if (!proposal) {
...
seiRouter.post('/update-proposals/:id/confirm', (req, res, next) => {
  ...
  try {
    const result = await confirmUpdateProposal(req.params.id as string, {
      fieldsToConfirm: req.body.fieldsToConfirm,
      confirmedBy: req.body.confirmedBy ?? (req as Request & { user?: { username: string } }).user?.username
    });
...
seiRouter.post('/update-proposals/:id/reject', (req, res, next) => {
  ...
  try {
    const result = await rejectUpdateProposal(
      req.params.id as string,
      req.body.rejectedBy ?? (req as Request & { user?: { username: string } }).user?.username
    );
