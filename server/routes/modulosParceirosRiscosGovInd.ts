 ...
 */
router.post('/:id/parceiros', requireAuth, async (req: Request, res: Response) => {
  try {
    const { id } = req.params;
    ...
    const validStatuses: DBStatusParceiro[] = ['Ativo', 'Apoiador', 'Consultor', 'Inativo'];
    let finalStatus: DBStatusParceiro = 'Ativo';
    if (status) {
    ...
    await col.updateOne(
      { _id: oid },
      { $push: { parceirosModulo: novoParceiro } }
    );
 ...
 */
router.put('/:id/parceiros/:parceiroId', requireAuth, async (req: Request, res: Response) => {
  try {
 ...
 */
router.delete('/:id/parceiros/:parceiroId', requireAuth, async (req: Request, res: Response) => {
  try {
    const { id, parceiroId } = req.params;
    ...
    await col.updateOne(
      { _id: oid },
      { $pull: { parceirosModulo: { id: parceiroId } } }
    );
 ...
 */
router.post('/:id/riscos', requireAuth, async (req: Request, res: Response) => {
  try {
    const { id } = req.params;
    ...
    // Validação de enums
    const validProbabilidades: DBProbabilidadeRisco[] = ['Baixa', 'Média', 'Alta'];
 ...
 */
router.put('/:id/riscos/:riscoId', requireAuth, async (req: Request, res: Response) => {
  try {
 ...
 */
router.delete('/:id/riscos/:riscoId', requireAuth, async (req: Request, res: Response) => {
  try {
    const { id, riscoId } = req.params;
 ...
 */
router.post('/:id/decisoes', requireAuth, async (req: Request, res: Response) => {
  try {
    const { id } = req.params;
    ...
    // Cria nova decisão
    const novaDecisao: DBDecisaoGovernanca = {
      id: randomUUID(),
      titulo: titulo.trim(),
      data: dateObj,
 ...
 */
router.put('/:id/decisoes/:decisaoId', requireAuth, async (req: Request, res: Response) => {
  try {
    ...
    await col.updateOne(
      { _id: oid },
      { $set: updates },
      { arrayFilters: [{ 'elem.id': decisaoId }] }
    );
 ...
 */
router.delete('/:id/decisoes/:decisaoId', requireAuth, async (req: Request, res: Response) => {
  try {
    const { id, decisaoId } = req.params;
 ...
 */
router.post('/:id/indicadores', requireAuth, async (req: Request, res: Response) => {
  try {
    const { id } = req.params;
    ...
    // Cria novo indicador
    const novoIndicador: DBIndicadorPesquisa = {
      id: randomUUID(),
      nome: nome.trim(),
      categoria: categoria.trim(),
      serie: (serie ?? []) as DBSerieDados[]
    };

    // Atualiza projeto
    await col.updateOne(
      { _id: oid },
      { $push: { indicadores: novoIndicador } }
    );
 ...
 */
router.put('/:id/indicadores/:indicadorId', requireAuth, async (req: Request, res: Response) => {
  try {
    ...
    if (serie !== undefined) {
      updates['indicadores.$[elem].serie'] = (serie ?? []) as DBSerieDados[];
    }
    ...
    await col.updateOne(
      { _id: oid },
      { $set: updates },
      { arrayFilters: [{ 'elem.id': indicadorId }] }
    );
 ...
 */
router.delete('/:id/indicadores/:indicadorId', requireAuth, async (req: Request, res: Response) => {
  try {
    const { id, indicadorId } = req.params;
    ...
    await col.updateOne(
      { _id: oid },
      { $pull: { indicadores: { id: indicadorId } } }
    );
