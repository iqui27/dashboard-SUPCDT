...
router.post('/', requireAuth, requireAdmin, async (req, res) => {
  try {
    const data = normalizeWifiEmpresaInput(req.body as unknown as WifiEmpresaInput);
...
router.put('/:id', requireAuth, requireAdmin, async (req, res) => {
  try {
    ...
    const success = await updateWifiEmpresa(req.params.id, normalizeWifiEmpresaInput(payload as unknown as WifiEmpresaInput));
    if (!success) {
      return res.status(404).json({ error: 'Empresa não encontrada' });
    }
