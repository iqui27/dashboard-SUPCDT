...
interface ProjectOverrideDoc {
  _id?: import('mongodb').ObjectId;
  projectId: string;
  overrides: Record<string, unknown>;
  needsSheetSync: boolean;
  lastOverrideAt: Date;
  lastSheetSyncAt?: Date | null;
  createdAt: Date;
  updatedAt: Date;
}
...
function normalizeHistoricoMovimentacoes(value: unknown) {
  ...
  return value.map(item => {
    ...
    if ('data' in entry) {
      entry.data = parseDateValue((entry as Record<string, unknown>).data);
    }
    return entry;
  });
}

function normalizeOverrides(payload: Record<string, unknown>) {
...
function serializeOverrides(overrides: Record<string, unknown>) {
...
function serializeOverrideDoc(doc: ProjectOverrideDoc) {
