...
import { SeiUpdateProposal } from '../types/sei.js';
import { SeiProjectSuggestion } from './seiProjectExtractor.js';
...
const DIFFABLE_FIELDS: (keyof SeiProjectSuggestion)[] = [
  'projeto',
  'numeroTermoFomento',
...
function normalizeForComparison(field: string, value: unknown): unknown {
  ...
  if (DATE_FIELDS.has(field)) {
    const parsed = parseDateValue(value as string);
...
export function computeDiff(
  ...
  for (const field of DIFFABLE_FIELDS) {
    ...
    if (Array.isArray(suggested) && (suggested as unknown[]).length === 0) continue;
    ...
    if (normalizedSuggested === null) continue;
    if (normalizedCurrent === normalizedSuggested) continue;
 ...
 */
export async function createUpdateProposal(
  processoSEI: string,
  ...
  success: boolean;
  proposalId?: string;
  proposal?: SeiUpdateProposal;
  hasChanges: boolean;
  error?: string;
}> {
  try {
    const db = await getDatabase();
    const proposalsCollection = db.collection<SeiUpdateProposal>(PROPOSALS_COLLECTION);
    ...
    // Create new proposal
    const proposal: SeiUpdateProposal = {
      processoSEI,
      linkedProjectId,
      status: 'pending',
      proposedChanges,
...
export async function confirmUpdateProposal(
  proposalId: string,
  options?: {
    fieldsToConfirm?: string[];  // subset of keys from proposedChanges; omit = all
    confirmedBy?: string;
  }
): Promise<{
  success: boolean;
        ...
        $set: {
          ...
          ...(isPartial ? {
            proposedChanges: Object.fromEntries(
              Object.entries(proposal.proposedChanges).filter(([k]) => !fieldsToApply.includes(k))
            )
          } : {})
        }
      }
 ...
 */
export async function rejectUpdateProposal(
  proposalId: string,
  rejectedBy?: string
  ...
  try {
    ...
    await proposalsCollection.updateOne(
      { _id: proposal._id },
      { $set: { status: 'rejected', rejectedAt: now, rejectedBy: rejectedBy ?? 'system', updatedAt: now } }
    );
...
export async function listUpdateProposals(options?: {
  status?: SeiUpdateProposal['status'];
  processoSEI?: string;
  linkedProjectId?: string;
  limit?: number;
  skip?: number;
}): Promise<{ proposals: SeiUpdateProposal[]; total: number }> {
  try {
    const db = await getDatabase();
    const collection = db.collection<SeiUpdateProposal>(PROPOSALS_COLLECTION);

    const filter: Record<string, unknown> = {};
    if (options?.status) filter.status = options.status;
    if (options?.processoSEI) filter.processoSEI = options.processoSEI;
    if (options?.linkedProjectId) filter.linkedProjectId = options.linkedProjectId;
 ...
 */
export async function getUpdateProposal(proposalId: string): Promise<SeiUpdateProposal | null> {
  try {
    const db = await getDatabase();
    const collection = db.collection<SeiUpdateProposal>(PROPOSALS_COLLECTION);
