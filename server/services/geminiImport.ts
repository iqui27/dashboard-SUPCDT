...
async function getCachedResult(hash: string): Promise<GeminiImportResult | null> {
...
function sanitizeSuggestion(raw: Record<string, unknown>): GeminiProjectSuggestion {
  const suggestion: GeminiProjectSuggestion = {};

  const assignString = (key: keyof GeminiProjectSuggestion) => {
    const value = raw[key];
    if (typeof value === 'string') {
      (suggestion as Record<string, unknown>)[key] = value.trim();
    }
  };

  const assignNumber = (key: keyof GeminiProjectSuggestion) => {
    const value = raw[key];
    if (typeof value === 'number') {
      (suggestion as Record<string, unknown>)[key] = value;
    } else if (typeof value === 'string') {
      const numeric = Number(value.replace(/[^0-9,-]/g, '').replace(',', '.'));
      if (!Number.isNaN(numeric)) {
        (suggestion as Record<string, unknown>)[key] = numeric;
      }
    }
  };
  ...
  if (Array.isArray(parlamentaresRaw)) {
    suggestion.parlamentares = parlamentaresRaw
      ...
      .filter((item): item is GeminiParlamentarInfo => Boolean(item));
  }
...
export async function extractProjectFromPdf(buffer: Buffer, fileName: string): Promise<GeminiImportResult> {
...
Retorne apenas JSON válido.`;
    ...
    let generationResponse: GeminiGenerationResponse | undefined;
    let lastError: Error | undefined;
    ...
    for (const candidate of modelSequence) {
      try {
        ...
        generationResponse = result as GeminiGenerationResponse;
    ...
    const suggestion = sanitizeSuggestion(parsed as Record<string, unknown>);

    const usageMetadata = generationResponse.response?.usageMetadata
      ? { ...generationResponse.response.usageMetadata }
      : undefined;

    const importResult: GeminiImportResult = {
      suggestion,
      usageMetadata
    };
