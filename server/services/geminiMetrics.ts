 ...
 */
export async function recordMetric(metric: Omit<GeminiMetric, 'timestamp'>): Promise<void> {
  try {
    const db = await getDatabase();
    const collection = db.collection<GeminiMetric>(METRICS_COLLECTION);
    ...
    if (metric.status === 'success') {
      console.log(
        `[Gemini Metrics] Success - Duration: ${metric.durationMs}ms, ` +
      ...
      console.error(
        `[Gemini Metrics] Error - Duration: ${metric.durationMs}ms, ` +
 ...
 */
export async function getRecentMetrics(): Promise<MetricsSummary> {
