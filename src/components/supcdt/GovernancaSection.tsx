    ...
    <>
      <div className="overflow-hidden rounded-[1.35rem] border border-white/80 bg-white/80 shadow-sm">
        <div className="flex items-center justify-between px-5 py-4">
          <div>
            ...
            <h3 className="mt-0.5 text-sm font-bold text-slate-900">Decisões de Governança</h3>
          </div>
          <Button
            variant="outline"
            size="sm"
                ...
                <li key={decisao.id} className="flex items-start justify-between gap-3 px-5 py-4">
                  <div className="min-w-0 flex-1">
                    <div className="flex items-center gap-2">
                      <span className="text-xs text-slate-400">{formatDate(decisao.data)}</span>
                      {decisao.responsavel && (
                    ...
                    <h4 className="mt-1 text-sm font-semibold text-slate-950">{decisao.titulo}</h4>
                    {decisao.descricao && (
                      <p className="mt-1 line-clamp-2 text-xs text-slate-600">{decisao.descricao}</p>
                    )}
                  </div>
                  <button
                    type="button"
      ...
      {isModalOpen && (
        <div className="fixed inset-0 z-[160] flex items-center justify-center overflow-y-auto bg-black/60 p-4 backdrop-blur-sm">
          <div className="w-full max-w-lg overflow-hidden rounded-[1.5rem] border border-white/80 bg-white shadow-[0_40px_100px_-30px_rgba(15,23,42,0.4)]">
            {/* Header */}
            <div className="sticky top-0 z-10 flex shrink-0 items-center justify-between border-b border-slate-100 bg-white/95 px-6 py-3.5 backdrop-blur-sm">
              <div className="flex items-center gap-2.5">
                ...
                <div>
                  ...
                  <h2 className="text-sm font-bold text-slate-900 leading-tight">Nova decisão</h2>
                </div>
              </div>
              <Button
                type="button"
            ...
            <form onSubmit={handleSubmit} className="space-y-4 bg-white px-6 pt-5 pb-0">
              <div className="space-y-1.5">
                ...
                <input
                  ...
                  className="h-9 w-full rounded-full border border-slate-200 bg-slate-50 px-3 text-sm text-slate-900 placeholder:text-slate-400 focus:outline-none focus:ring-2 focus:ring-violet-500/40"
                  value={titulo}
              ...
              <div className="grid grid-cols-2 gap-3">
                <div className="space-y-1.5">
                  ...
                  <input
                    required
                    type="date"
                    className="h-9 w-full rounded-full border border-slate-200 bg-slate-50 px-3 text-sm text-slate-900 focus:outline-none focus:ring-2 focus:ring-violet-500/40"
                    value={data}
              ...
              <div className="space-y-1.5">
                ...
                <textarea
                  ...
                  className="w-full rounded-[0.9rem] border border-slate-200 bg-slate-50 px-3 py-2 text-sm text-slate-900 placeholder:text-slate-400 focus:outline-none focus:ring-2 focus:ring-violet-500/40 resize-none"
                  value={descricao}
              ...
              {/* Footer */}
              <div className="flex justify-end gap-2 border-t border-slate-100 py-4">
                <Button
                  type="button"
                  variant="outline"
                  onClick={() => setIsModalOpen(false)}
                  disabled={loading}
                  className="h-8 rounded-full border-slate-200 px-4 text-xs text-slate-600"
                >
                  Cancelar
                </Button>
                <Button
