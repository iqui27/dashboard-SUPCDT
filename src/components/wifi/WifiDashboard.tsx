  ...
  return (
    <div className="space-y-6">
      <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
        {[
          { label: 'Total de Pontos', value: stats.totalPontos, helper: 'Pontos cadastrados na operação', icon: Wifi, tone: 'bg-sky-100 text-sky-700' },
          { label: 'Pontos Críticos', value: stats.pontosCriticos, helper: 'Fila com criticidade máxima', icon: ShieldAlert, tone: 'bg-rose-100 text-rose-700' },
          { label: 'Manutenção Pendente', value: stats.manutencaoPendente, helper: 'Pontos em corretiva ou pendente', icon: Wrench, tone: 'bg-amber-100 text-amber-700' },
          { label: 'Incidentes Abertos', value: stats.incidentesAbertos, helper: 'Volume atual de ocorrências registradas', icon: AlertTriangle, tone: 'bg-slate-100 text-slate-700' }
        ].map((item) => (
          <div key={item.label} className="rounded-[1.75rem] border border-white/80 bg-white/85 p-6 shadow-[0_20px_70px_-42px_rgba(15,23,42,0.35)]">
            ...
            <h3 className="mt-4 text-3xl font-bold text-slate-950">{item.value}</h3>
      ...
      <div className="grid gap-6 xl:grid-cols-[minmax(0,1fr)_minmax(360px,0.9fr)]">
        <div className="rounded-[1.75rem] border border-white/80 bg-white/85 p-6 shadow-[0_24px_70px_-42px_rgba(15,23,42,0.35)]">
          <div className="flex items-center justify-between">
            <div>
              <h3 className="text-xl font-semibold text-slate-950">Distribuição operacional</h3>
              <p className="mt-1 text-sm text-slate-500">Status dos pontos já cadastrados no Wi‑Fi Social.</p>
            </div>
          ...
          <div className="mt-6 grid gap-6 lg:grid-cols-[minmax(0,0.9fr)_minmax(0,1.1fr)]">
            ...
            <div className="space-y-3">
              ...
              <div className="rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3">
                <p className="text-xs uppercase tracking-[0.16em] text-slate-500">Usuários conectados</p>
                <p className="mt-1 text-lg font-semibold text-slate-950">{stats.totalUsuarios}</p>
              </div>
              <div className="rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3">
                <p className="text-xs uppercase tracking-[0.16em] text-slate-500">Velocidade média</p>
                <p className="mt-1 text-lg font-semibold text-slate-950">{stats.velocidadeMedia || 0} Mbps</p>
              </div>
            </div>
          </div>
        </div>
          ...
          <div className="mt-5 space-y-3">
            {stats.filaAtencao.length ? (
              stats.filaAtencao.map((point) => {
                ...
                return (
                  <div key={point.id} className="rounded-2xl border border-slate-200 bg-slate-50 px-4 py-4">
                    ...
                    <p className="mt-3 font-semibold text-slate-950">{point.nome}</p>
                    <p className="mt-1 text-sm text-slate-600">{point.regiaoAdministrativa}</p>
          ...
          <div className="mt-5 space-y-3">
            {stats.regioesCriticas.length ? (
              stats.regioesCriticas.map((item) => (
                <div key={item.regiaoAdministrativa} className="rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3">
                  <div className="flex items-center justify-between">
                    <p className="font-medium text-slate-950">{item.regiaoAdministrativa}</p>
                    <span className="text-sm text-slate-600">{item.total} ponto(s)</span>
                  </div>
      ...
      <div className="grid gap-6 xl:grid-cols-[minmax(0,1fr)_minmax(360px,0.9fr)]">
        <div className="rounded-[1.75rem] border border-white/80 bg-white/85 p-6 shadow-[0_24px_70px_-42px_rgba(15,23,42,0.35)]">
          <h3 className="text-xl font-semibold text-slate-950">Resumo rápido da rede</h3>
          <div className="mt-5 grid gap-3 sm:grid-cols-2">
            {points.slice(0, 4).map((point) => (
              <div key={point.id} className="rounded-[1.5rem] border border-slate-200 bg-slate-50 px-4 py-4">
                <p className="font-semibold text-slate-950">{point.nome}</p>
                <p className="mt-1 text-sm text-slate-600">{point.regiaoAdministrativa}</p>
                <p className="mt-3 text-xs uppercase tracking-[0.16em] text-slate-500">Cobertura</p>
                <p className="mt-1 text-sm font-medium text-slate-900">{point.coberturaRaioMetros} m</p>
              </div>
            ))}
            {points.length === 0 && (
              <div className="sm:col-span-2 rounded-[1.5rem] border border-dashed border-slate-200 bg-slate-50 px-5 py-10 text-center text-sm text-slate-500">
        ...
        <div className="rounded-[1.75rem] border border-white/80 bg-white/85 p-6 shadow-[0_24px_70px_-42px_rgba(15,23,42,0.35)]">
          <h3 className="text-xl font-semibold text-slate-950">Últimas movimentações</h3>
          <div className="mt-5 space-y-3">
            {stats.recentes.length ? (
              stats.recentes.map((point) => (
                <div key={point.id} className="rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3">
                  <p className="font-medium text-slate-950">{point.nome}</p>
                  <p className="mt-1 text-sm text-slate-600">{point.regiaoAdministrativa}</p>
