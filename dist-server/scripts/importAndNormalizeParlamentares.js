...
const PARLAMENTARES_NORMALIZATION_MAP = {
...
function normalizarNomeParlamentar(nome) {
    ...
    return PARLAMENTARES_NORMALIZATION_MAP[nomeLimpo] || nome;
}
// Função para separar múltiplos parlamentares
function separarMultiplosParlamentares(texto) {
    if (!texto)
        return [];
    const textoLimpo = texto.trim();
    if (!textoLimpo || textoLimpo.toLowerCase() === 'não se aplica')
        return [];
...
async function importAndNormalizeParlamentares() {
    ...
    try {
        ...
        for (const project of projetosFinal) {
        ...
        for (const project of amostra) {
            ...
            project.emendasParlamentares.forEach((emenda, index) => {
