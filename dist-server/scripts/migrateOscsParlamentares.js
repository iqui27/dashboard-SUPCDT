...
async function migrateOSCs() {
    ...
    try {
        ...
        const nomeParaParlamentarId = new Map();
        const nomeNormalizadoMap = {
        ...
        const oscs = await oscsCollection.find({}).toArray();
        console.log(`📊 Encontradas ${oscs.length} OSCs para processar`);
        let atualizadas = 0;
        let naoEncontrados = 0;
        const parlamentaresNaoEncontrados = new Set();
        for (const osc of oscs) {
            ...
            if (parlamentarId) {
                ...
                await oscsCollection.updateOne({ _id: osc._id }, {
                    $set: {
 ...
 */
async function adicionarParlamentaresFaltantes(nomes) {
    const client = new MongoClient(MONGODB_URI);
    try {
        await client.connect();
        const db = client.db(DB_NAME);
