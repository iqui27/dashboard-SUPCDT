...
function normalizeEvidencias(values) {
    if (!Array.isArray(values)) {
        return [];
    }
    return values
...
function isGenericIntegrationValue(value) {
    if (!value) {
        return true;
    }
...
function summarizeEvidencias(evidencias) {
...
function resolveHistoricoUsuario(usuario) {
    return {
...
function buildProjetoAuditSnapshot(projeto) {
    ...
    return {
        ...
        cronograma: projeto.cronograma?.totalTrimestres && Number(projeto.cronograma.totalTrimestres) > 0
        ...
        precisaAcao: typeof monitoramento.precisaAcao === 'boolean'
            ? (monitoramento.precisaAcao ? 'Sim' : 'Não')
            : null,
        incidentesAbertos: typeof monitoramento.incidentesAbertos === 'number'
...
function buildProjetoHistoricoAlteracoes(anterior, atual) {
...
function buildProjetoHistoricoCriacao(projeto) {
...
function buildResumoAtualizacao(alteracoes) {
...
function buildHistoricoItem(payload) {
    return {
...
function buildIntegrationKey(projeto, id) {
...
function deriveStatusOperacional(projeto) {
    const status = sanitizeText(projeto.status)?.toLowerCase() ?? '';
    if (status.includes('encerr'))
        return 'Encerrado';
    if (status.includes('critico') || status.includes('paralis') || status.includes('cancel'))
        return 'Crítico';
    if (status.includes('ativo') || status.includes('andamento') || status.includes('assinado'))
        return 'Operando';
    if (status.includes('implant'))
        return 'Em implantação';
    return 'Planejado';
}
function deriveNivelRisco(projeto, diasRestantes) {
    const status = sanitizeText(projeto.status)?.toLowerCase() ?? '';
...
function deriveSaudeEntrega(nivelRisco, statusOperacional) {
    if (nivelRisco === 'Crítico' || statusOperacional === 'Crítico')
        return 'Risco';
    if (nivelRisco === 'Alto' || statusOperacional === 'Em implantação')
        return 'Observação';
    return 'Saudável';
}
function buildMonitoringGaps(monitoramento) {
    const lacunas = [];
...
function buildMonitoramentoOperacional(projeto) {
...
function normalizeMonitoramentoInput(input, fallbackResponsavel) {
...
export function mapProjetoToApi(projeto) {
...
export function normalizeProjetoInput(input) {
...
export async function createProjeto(projetoData, usuario) {
...
export async function registrarProjetoHistorico(projetoId, payload) {
    const db = await getDatabase('dashboard_supcdt');
    const collection = db.collection(COLLECTION_NAME);
    try {
        ...
        await collection.updateOne({ _id: new ObjectId(projetoId) }, { $set: { historicoAlteracoes: historicoAtualizado } });
    }
    catch {
        // Mantemos o fluxo principal resiliente mesmo se a escrita do histórico falhar.
    }
}
export async function updateProjeto(id, updateData, usuario) {
    const db = await getDatabase('dashboard_supcdt');
    const collection = db.collection(COLLECTION_NAME);
    try {
        ...
        const { _id, createdAt, historicoAlteracoes, ...fieldsToUpdate } = updateData;
