...
import { normalizeProjectPayload, serializeProjectDoc, parseDateValue, parseMonetaryValue, normalizeStatusProjeto, normalizeCategoria, parseStringArray } from './projectNormalization.js';
// Collection names
 ...
 */
function normalizeTextFieldsToUpper(suggestion) {
    const normalized = { ...suggestion };
    const normalizedAsRecord = normalized;
    for (const field of TEXT_FIELDS_TO_UPPERCASE) {
        if (normalizedAsRecord[field] && typeof normalizedAsRecord[field] === 'string') {
            normalizedAsRecord[field] = normalizedAsRecord[field].toUpperCase();
        }
    }
    ...
    if (normalized.regioesAdministrativas && Array.isArray(normalized.regioesAdministrativas)) {
        normalized.regioesAdministrativas = normalized.regioesAdministrativas.map(r => typeof r === 'string' ? r.toUpperCase() : r);
    }
 ...
 */
export async function persistSuggestion(extractionResult, options) {
    const startTime = Date.now();
    const suggestionId = generateSuggestionId();
    try {
        const db = await getDatabase();
        const collection = db.collection(SUGGESTIONS_COLLECTION);
        const now = new Date();
        ...
        // Upsert suggestion
        if (existing) {
            await collection.updateOne({ processoSEI: extractionResult.processoSEI }, { $set: record });
 ...
 */
export async function approveSuggestion(suggestionIdOrProcessoSEI, approvedBy) {
    const startTime = Date.now();
    try {
        const db = await getDatabase();
        ...
        const oscsCollection = db.collection(OSCS_COLLECTION);
        ...
        if (existingProject) {
            ...
            projectId = existingProject._id instanceof ObjectId
                ...
                : existingProject.id || String(existingProject._id);
            ...
            await projectsCollection.updateOne({ _id: existingProject._id }, { $set: mergedPayload });
            ...
            await projectsCollection.updateOne({ _id: insertResult.insertedId }, { $set: { id: projectId } });
            console.log(`[Auto Populate] Created new project ${projectId} for process ${suggestion.processoSEI}`);
            isNewProject = true;
        }
        // Update suggestion status to approved
        await suggestionsCollection.updateOne({ _id: suggestion._id }, {
            $set: {
                status: 'approved',
                approvedAt: now,
        ...
        return {
            ...
            projectId,
            isNewProject,
            project: finalProject ? serializeProjectDoc(finalProject) : undefined,
            suggestion: {
                ...suggestion,
                status: 'approved',
                approvedAt: now,
 ...
 * Reject a suggestion
 */
export async function rejectSuggestion(suggestionIdOrProcessoSEI, rejectedBy, reason) {
    try {
        const db = await getDatabase();
        const collection = db.collection(SUGGESTIONS_COLLECTION);
        const now = new Date();
        ...
        await collection.updateOne({ _id: suggestion._id }, {
            $set: {
                status: 'rejected',
                rejectedAt: now,
 ...
 */
export async function getSuggestion(suggestionIdOrProcessoSEI) {
    try {
        const db = await getDatabase();
        const collection = db.collection(SUGGESTIONS_COLLECTION);
 ...
 */
export async function deleteSuggestion(suggestionIdOrProcessoSEI) {
    try {
        const db = await getDatabase();
        const collection = db.collection(SUGGESTIONS_COLLECTION);
        const suggestion = await collection.findOne({
 ...
 */
export async function projectExistsForProcess(processoSEI) {
    try {
        const db = await getDatabase();
 ...
 */
export async function updateSuggestionFields(id, updates) {
    try {
        const db = await getDatabase();
        const collection = db.collection(SUGGESTIONS_COLLECTION);
        // Find the suggestion
