...
import { z } from 'zod';
import { ensureApiKey, retryWithBackoff, extractJsonObject, calculateHash, getCachedResult, saveCachedResult, validateWithSchema, createExtractionError, recordExtractionMetric, Semaphore, MAX_RETRIES } from './geminiUtils.js';
import { SEI_DOCUMENT_TYPES } from '../types/sei.js';
...
const PREFERRED_DOCUMENT_TYPES = [
 ...
 */
function selectCandidateDocuments(documents) {
    ...
    const preferred = nonErrored.filter(doc => PREFERRED_DOCUMENT_TYPES.includes(doc.tipo));
    ...
    return nonErrored.filter(doc => !EXCLUDED_DOCUMENT_TYPES.includes(doc.tipo) &&
 ...
 */
function sanitizeProjectSuggestion(raw) {
    const suggestion = {};
    // String fields
    const stringFields = [
    ...
    if (Array.isArray(emendasRaw)) {
        suggestion.emendasParlamentares = emendasRaw
            ...
            if (typeof obj.valor === 'number' && !Number.isNaN(obj.valor)) {
                ...
                const parsed = Number(obj.valor.replace(/[^\d,-]/g, '').replace(',', '.'));
                if (!Number.isNaN(parsed))
                    valor = parsed;
            }
            return {
 ...
 */
export async function extractSeiProjectSuggestion(processoSEI, documents, options) {
    ...
    if (!options?.forceRefresh) {
        const cached = await getCachedResult(candidateHash, SEI_PROJECT_CACHE_COLLECTION);
        if (cached) {
...
Retorne APENAS o JSON válido, sem markdown, sem explicações.`;
        ...
        try {
            ...
            const error = createExtractionError('validation', parseError instanceof Error ? parseError.message : 'JSON malformado na resposta', retryCount);
            throw error;
        }
        ...
        // Sanitize suggestion
        const suggestion = sanitizeProjectSuggestion(parsed);
 ...
 */
export async function batchExtractSeiProjectSuggestions(inputs) {
    const results = [];
    // Process each input sequentially (rate limiting handled by extractionSemaphore)
    for (const input of inputs) {
        const result = await extractSeiProjectSuggestion(input.processoSEI, input.documents, { linkedProjectId: input.linkedProjectId });
        results.push(result);
    }
    return results;
}
 ...
 */
export async function clearSeiProjectCache(_processoSEI) {
    try {
