...
function calculateLinkHash(link) {
    return crypto.createHash('sha256').update(link).digest('hex');
}
async function getCachedResult(link) {
    if (!CACHE_ENABLED)
        return null;
    try {
...
async function saveCachedResult(link, tipo, extracted) {
    if (!CACHE_ENABLED)
        return;
    try {
        const db = await getDatabase();
        ...
        await collection.updateOne({ hash: calculateLinkHash(link) }, {
            $set: {
                hash: calculateLinkHash(link),
                link,
                tipo,
...
async function extractMetadataFromDocument(document, apiKey, _retryCount = 0) {
 ...
 */
async function extractSingleDocument(document, apiKey) {
 ...
 */
export async function extractFromSeiDocuments(processoSEI, documents) {
    const startTime = Date.now();
    const correlationId = crypto.randomUUID();
    let totalRetries = 0;
    const errors = [];
 ...
 */
export async function storeSeiProcess(processoSEI, documents, createdBy) {
    try {
        const db = await getDatabase();
        const collection = db.collection('sei_processes');
        const now = new Date();
        await collection.updateOne({ processoSEI }, {
            $set: {
                processoSEI,
                documents,
 ...
 */
export async function getStoredSeiProcess(processoSEI) {
    try {
        const db = await getDatabase();
        const collection = db.collection('sei_processes');
