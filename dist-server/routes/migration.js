...
import { fetchSheetProjects, fetchParlamentaresFromSheet } from '../services/sheetFomentos.js';
const migrationRouter = Router();
// GET /api/migration/status - Verificar status da migração
migrationRouter.get('/status', async (_req, res) => {
    try {
        ...
        const migratedProjectNames = new Set(dbProjects.map((p) => p.projeto?.toLowerCase().trim()).filter(Boolean));
...
migrationRouter.post('/migrate-all', async (_req, res) => {
    try {
        const db = await getDatabase();
        const sheetResult = await fetchSheetProjects();
        const sheetProjects = sheetResult.projects;
...
migrationRouter.post('/migrate/:projectName', async (req, res) => {
    try {
        const { projectName } = req.params;
        const sheetResult = await fetchSheetProjects();
        const sheetProjects = sheetResult.projects;
        const project = sheetProjects.find((p) => p.raw.projeto?.toLowerCase().trim() === projectName.toLowerCase().trim());
        if (!project) {
...
migrationRouter.get('/analyze-existing', async (_req, res) => {
    try {
        const db = await getDatabase();
        const sheetResult = await fetchSheetProjects();
        const sheetProjects = sheetResult.projects;
        ...
        for (const dbProject of dbProjects) {
            const sheetProject = sheetProjects.find((p) => p.raw.projeto?.toLowerCase().trim() === dbProject.projeto?.toLowerCase().trim());
            if (!sheetProject)
                continue;
            const missingFields = [];
            const emptyFields = [];
...
migrationRouter.post('/populate-fields', async (req, res) => {
    try {
        ...
        for (const projectId of selectedProjects) {
            try {
                ...
                // Atualizar no BD
                await db.collection('custom_projects').updateOne({ _id: new ObjectId(projectId) }, { $set: updateDoc });
                results.updated++;
            }
            catch (error) {
...
migrationRouter.post('/replace-fields', async (req, res) => {
    try {
...
migrationRouter.get('/full-comparison', async (_req, res) => {
    try {
        const db = await getDatabase();
        const sheetResult = await fetchSheetProjects();
        const sheetProjects = sheetResult.projects;
...
migrationRouter.post('/sync-to-sheet', async (req, res) => {
    try {
...
function normalizeValue(value) {
    ...
    if (typeof value === 'object') {
        if (value.nome)
            return normalizeValue(value.nome);
        return JSON.stringify(value).toLowerCase();
    }
    return value;
}
// GET /api/migration/frontend-comparison - Comparar dados exatos do frontend
migrationRouter.get('/frontend-comparison', async (_req, res) => {
    try {
        const db = await getDatabase();
        const sheetResult = await fetchSheetProjects();
        const sheetProjects = sheetResult.projects;
        ...
        const sheetDataFormatted = sheetProjects.map(sheetProject => {
            ...
            frontendFields.forEach(field => {
                if (field === 'id' || field === 'origin')
                    return;
                let value = sheetProject.raw[field];
                // Converter datas
        ...
        const dbDataFormatted = dbProjects.map(dbProject => {
            const formatted = { origin: 'mongodb' };
            frontendFields.forEach(field => {
                if (field === 'origin')
                    return;
        ...
        sheetDataFormatted.forEach(sheetProject => {
            const dbProject = dbDataFormatted.find(db => db.projeto?.toLowerCase().trim() === sheetProject.projeto?.toLowerCase().trim());
            if (dbProject) {
...
migrationRouter.post('/sync-frontend-fields', async (req, res) => {
    try {
...
migrationRouter.get('/sync-conflicts', async (_req, res) => {
    try {
        const db = await getDatabase();
        const sheetResult = await fetchSheetProjects();
        const sheetProjects = sheetResult.projects;
...
migrationRouter.post('/approve-sync', async (req, res) => {
    try {
        ...
        for (const conflictIndex of selectedConflicts) {
            try {
                ...
                if (!conflict)
                    continue;
                const sheetProject = sheetProjects.find((p) => p.raw.projeto?.toLowerCase().trim() === conflict.projectName.toLowerCase().trim());
                if (!sheetProject) {
                ...
                await db.collection('custom_projects').updateOne({ _id: new ObjectId(conflict.projectId) }, { $set: updateDoc });
                results.updated++;
            }
            catch (error) {
...
migrationRouter.post('/sync-parlamentares', async (_req, res) => {
    try {
        const db = await getDatabase();
        ...
        for (const sheetProject of parlamentarProjects) {
            try {
                ...
                let dbProject = dbProjects.find((p) => p.projeto?.toLowerCase().trim() === sheetProject.projectName.toLowerCase().trim());
                ...
                await db.collection('custom_projects').updateOne({ id: dbProject.id }, // Usar o campo id custom em vez de _id
                {
                    $set: {
...
migrationRouter.post('/sync-all-parlamentares', async (_req, res) => {
    try {
        const db = await getDatabase();
        const sheetResult = await fetchSheetProjects();
        const sheetProjects = sheetResult.projects;
        ...
        for (const sheetProject of sheetProjects) {
            try {
                ...
                const dbProject = dbProjects.find((p) => p.projeto?.toLowerCase().trim() === sheetProject.raw.projeto?.toLowerCase().trim());
                if (!dbProject) {
                    notFound++;
                ...
                await db.collection('custom_projects').updateOne({ id: dbProject.id }, {
                    $set: {
                        parlamentares: parlamentaresArray,
                        parlamentar: parlamentaresString,
                        updatedAt: new Date()
