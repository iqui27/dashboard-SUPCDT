...
oscsRouter.put('/:id', async (req, res) => {
    try {
        const { id } = req.params;
        const input = parseOscInput(req.body);
...
oscsRouter.delete('/:id', async (req, res) => {
    try {
        const { id } = req.params;
        await deleteOsc(id);
        res.status(204).send();
...
oscsRouter.post('/upsert', async (req, res) => {
    try {
        const input = parseOscInput(req.body);
