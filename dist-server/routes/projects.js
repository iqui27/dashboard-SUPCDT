...
function normalizeProjectPayload(payload) {
    ...
    if (Array.isArray(doc.historicoMovimentacoes)) {
        doc.historicoMovimentacoes = doc.historicoMovimentacoes.map(item => ({
            ...item,
...
function serializeProject(doc) {
    const { _id, id: legacyId, ...rest } = doc;
...
projectsRouter.post('/import', requireAuth, upload.single('file'), async (req, res) => {
    try {
...
projectsRouter.get('/', async (_req, res) => {
    try {
        ...
        const processedProjects = projects.map(project => {
            const processed = normalizeProjectPayload(project);
            return serializeProject(processed);
        });
...
projectsRouter.get('/:id', async (req, res) => {
    try {
        ...
        const processed = normalizeProjectPayload(project);
        res.json(serializeProject(processed));
    }
    catch (error) {
...
projectsRouter.post('/', requireAuth, async (req, res) => {
    try {
        ...
        const projeto = payload.projeto?.trim();
        if (!projeto) {
        ...
        const idValue = isMongoObjectId(insertedId)
            ...
            : String(insertedId);
        const saved = await collection.findOneAndUpdate(isMongoObjectId(insertedId)
            ? { _id: insertedId }
            : { _id: insertedId }, { $set: { id: idValue } }, { returnDocument: 'after' });
        if (!saved) {
            return res.status(500).json({ error: 'Failed to save project' });
        }
        res.status(201).json(serializeProject(saved));
    }
    catch (error) {
    ...
    try {
        ...
        if (!incomingHistory || incomingHistory.length === 0) {
            ...
            if (preservedHistory.length > 0) {
                doc.historicoMovimentacoes = preservedHistory;
            }
            else {
                delete doc.historicoMovimentacoes;
            }
        }
        else {
            ...
            doc.historicoMovimentacoes = mergedHistory;
        }
        const combinedFilter = { $or: orFilters };
        ...
        res.json(serializeProject(updatedDoc));
    }
    catch (error) {
